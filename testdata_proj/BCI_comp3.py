...
#    print "starting " + str(i)
    # training data:
#    with open(DATA_DIR + "traindata/trial" + str(i) + "instances.csv", "rb") as instance_file:
#        instance_csv = csv.reader(instance_file)
#        for row in instance_csv:
#            row = eval(row[0])
#            train_instances.append(row)
#    with open(DATA_DIR + "traindata/trial" + str(i) + "_labels.csv", "rb") as label_file:
#        label_csv = csv.reader(label_file)
#        for row in label_csv:
#            train_labels.append(row)

    # test data:
#    with open(DATA_DIR + "evaluationdata/trial" + str(i) + "instances.csv", "rb") as instance_file:
#        instance_csv = csv.reader(instance_file)
#        for row in instance_csv:
#            test_instances.append(row)
#    with open(DATA_DIR + "evaluationdata/trial" + str(i) + "_labels.csv", "rb") as label_file:
#        label_csv = csv.reader(label_file)
#        for row in label_csv:
#            test_labels.append(row)


# sort train instances by label
#sorted_by_labels = [[], [], []]
#for i, label in enumerate(train_labels):
#    sorted_by_labels[int(label[0])].append(train_instances[i])
#sorted_by_labels = np.ndarray(sorted_by_labels)
#pdb.set_trace()
